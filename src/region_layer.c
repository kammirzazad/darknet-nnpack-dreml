...
void  adjustRegionLossesDREML(const region_layer l, network_state state, int index, int i, int j, int n)
{
    ...
    {	
        ...
        {
            l.delta[index + 4] = l.anchor_boxes[n] * l.object_scale * l.output[index + 4] * logistic_gradient(l.output[index + 4]);
            ...
            for(class_id = 0; class_id < l.classes; ++class_id)
            {
                ...
                {
                    l.delta[index2] = l.anchor_boxes[n] * l.class_counts[class_id] * l.class_scale * prob; //(1.0 - l.output[index2]);
                }
                else
                {
                    l.delta[index2] = l.anchor_boxes[n] * l.class_counts[class_id] * l.class_scale * prob; //* (0.0 - l.output[index2]);
                }
            }
       }
       else
       {
            l.delta[index + 4] = l.anchor_boxes[n] * l.noobject_scale * l.output[index + 4] * logistic_gradient(l.output[index + 4]);

            for(coord_id = 0; coord_id < l.coords; coord_id++)
            {
                l.delta[index + coord_id] = l.anchor_boxes[n] * l.coord_scale * 0.0; //EPSILON;
            ...
            for(class_id = 0; class_id < l.classes; ++class_id)
            {
                ...
                l.delta[index2] = l.anchor_boxes[n] * l.class_counts[class_id] * l.class_scale * 0.0; //EPSILON;
            }
        }
    }
}
