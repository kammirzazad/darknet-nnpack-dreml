    ...
    {	
        if(l.output[index + 4] > state.dreml_det_thresh)
        {
            ...
            for(class_id = 0; class_id < l.classes; ++class_id)
            {
                ...
                {
                    l.delta[index2] = l.anchor_boxes[n] * l.class_counts[class_id] * l.class_scale * l.output[index2];
                }
                else
                {
                    l.delta[index2] = l.anchor_boxes[n] * l.class_counts[class_id] * l.class_scale * 0.0; //* (0.0 - l.output[index2]);
                }
            }
       }
       else
       {
            l.delta[index + 4] = l.anchor_boxes[n] * l.noobject_scale /* (0.0 - l.output[index + 4])*/ * logistic_gradient(l.output[index + 4]) * 0.0;
