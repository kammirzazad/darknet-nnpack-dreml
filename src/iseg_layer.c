...
void forward_iseg_layer(const layer l, network_state state)
{
    ...
    for (b = 0; b < l.batch; ++b){
        ...
        for(i = 0; i < 90; ++i){
            ...
            int c = state.truth[b*l.truths + i*(l.w*l.h+1)];
            if(c < 0) break;
            // add up metric embeddings for each instance
            for(k = 0; k < l.w*l.h; ++k){
                int index = b*l.outputs + c*l.w*l.h + k;
                float v = state.truth[b*l.truths + i*(l.w*l.h + 1) + 1 + k];
                if(v){
...
void backward_iseg_layer(const layer l, network_state state)
{
...
void forward_iseg_layer_gpu(const layer l, network_state state)
{
    copy_gpu(l.batch*l.inputs, state.input_gpu, 1, l.output_gpu, 1);
    int b;
    for (b = 0; b < l.batch; ++b){
    ...
    forward_iseg_layer(l, state);
    cuda_push_array(l.delta_gpu, l.delta, l.batch*l.outputs);
}

void backward_iseg_layer_gpu(const layer l, network_state state)
{
    int b;
    for (b = 0; b < l.batch; ++b){
    ...
    axpy_gpu(l.batch*l.inputs, 1, l.delta_gpu, 1, state.delta_gpu, 1);
}
#endif
